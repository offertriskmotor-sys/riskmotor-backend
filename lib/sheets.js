...
/**
 * SHEET CONTRACT (based on your current "Fält/Värde" INPUT layout):
...
export async function writeInputReadResult({
  ...
  // 2) Main inputs -> INPUT!B2:B12 (values column)
  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: "INPUT!B2:B12",
    valueInputOption: "USER_ENTERED",
    requestBody: {
      values: [
        ...
        [toPrismodell(prismodell)],          // B6  TEXT
